...
func (s *Service) GrantAccessToken(client *entity.OauthClient, user *entity.OauthUser, expiresIn int, scope string) (*entity.OauthAccessToken, error) {
	// Begin a transaction
	tx := s.db.Begin()
	...
	accessToken := entity.NewOauthAccessToken(client, user, expiresIn, scope)
