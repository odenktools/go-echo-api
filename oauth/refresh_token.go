...
func (s *Service) GetOrCreateRefreshToken(client *entity.OauthClient, user *entity.OauthUser, expiresIn int, scope string) (*entity.OauthRefreshToken, error) {
	// Try to fetch an existing refresh token first
	refreshToken := new(entity.OauthRefreshToken)
	query := entity.OauthRefreshTokenPreload(s.db).Where("client_id = ?", client.ID)
	...
	if expired || !found {
		refreshToken = entity.NewOauthRefreshToken(client, user, expiresIn, scope)
...
func (s *Service) GetValidRefreshToken(token string, client *entity.OauthClient) (*entity.OauthRefreshToken, error) {
	// Fetch the refresh token from the database
	refreshToken := new(entity.OauthRefreshToken)
	notFound := entity.OauthRefreshTokenPreload(s.db).Where("client_id = ?", client.ID).
...
func (s *Service) getRefreshTokenScope(refreshToken *entity.OauthRefreshToken, requestedScope string) (string, error) {
	var (
