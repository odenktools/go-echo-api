...
import (
	...
	"go-echo-api/entity"
	"go-echo-api/utils"
	pass "go-echo-api/utils/password"
)
...
func (s *Service) FindUserByUsername(username string) (*entity.OauthUser, error) {
	// Usernames are case insensitive
	user := new(entity.OauthUser)
...
func (s *Service) CreateUser(roleID, username, password string) (*entity.OauthUser, error) {
...
func (s *Service) CreateUserTx(tx *gorm.DB, roleID, username, password string) (*entity.OauthUser, error) {
	return s.createUserCommon(tx, roleID, username, password)
}

// SetPassword sets a user password
func (s *Service) SetPassword(user *entity.OauthUser, password string) error {
	return s.setPasswordCommon(s.db, user, password)
}

// SetPasswordTx sets a user password in a transaction
func (s *Service) SetPasswordTx(tx *gorm.DB, user *entity.OauthUser, password string) error {
	return s.setPasswordCommon(tx, user, password)
}

// AuthUser authenticates user
func (s *Service) AuthUser(username, password string) (*entity.OauthUser, error) {
	// Fetch the user
...
// UpdateUsername ...
func (s *Service) UpdateUsername(user *entity.OauthUser, username string) error {
	if username == "" {
...
// UpdateUsernameTx ...
func (s *Service) UpdateUsernameTx(tx *gorm.DB, user *entity.OauthUser, username string) error {
	return s.updateUsernameCommon(tx, user, username)
}

func (s *Service) createUserCommon(db *gorm.DB, roleID, username, password string) (*entity.OauthUser, error) {
...
func (s *Service) setPasswordCommon(db *gorm.DB, user *entity.OauthUser, password string) error {
	if len(password) < MinPasswordLength {
	...
	return db.Model(user).UpdateColumns(entity.OauthUser{
		Password:    utils.StringOrNull(string(passwordHash)),
		MyGormModel: entity.MyGormModel{UpdatedAt: time.Now().UTC()},
	}).Error
}

func (s *Service) updateUsernameCommon(db *gorm.DB, user *entity.OauthUser, username string) error {
	if username == "" {
		return ErrCannotSetEmptyUsername
	}
